...
JOIN concession c on v.concession_ville=c.ville
JOIN pays p on c.pays=p.nom
where prix_journalierht * (1 + p.taxe / 100) >= ALL
(select prix_journalierht * (1 + taxe / 100) from voiture v2 
join concession c2 on v2.concession_ville=c2.ville 
join pays p2 on c2.pays=p2.nom);
